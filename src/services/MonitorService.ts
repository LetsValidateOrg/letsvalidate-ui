...
export const MonitorService = {
  ...
  async getMonitoredCerts(): Promise<MonitoredCertificateResponse> {
    ...
    if (fetchResponse.ok) {
      ...
      const cachedBrowserStateString = AuthService.getCookie('LETSVAL_USER_STATE_CACHE')
  ...
  async removeMonitor(monitorIdToCancel: string): Promise<MonitoredCertificateResponse | null> {
    const accessToken = AuthService.getAccessToken()
    if (!accessToken) {
      return null
    }
    ...
    const constructedRequestUrl =
      monitoredUrlApiEndpoint + '?' + new URLSearchParams({ monitor_id: monitorIdToCancel })
...
function scrubHostnameOrIp(url: string): string {
  ...
  for (const currPrefix of prefixesToRemove) {
    if (url.startsWith(currPrefix)) {
      ...
      console.log(
        'Found prefix to remove "' + currPrefix + '", after removing it have "' + url + '"'
      )
    }
  }
